...
export const updateGenreFilteredData = genreData => {
  const myElement = document.querySelector(".filter-movies");
  myElement.firstChild.nodeValue = "Filter by genre";
...
export const addToWatch = imdbID => {
  return (dispatch, getState) => {
    fetch(`${Communicators.omdbURL}&i=${imdbID}`)
      .then(response => response.json())
      .then(myJson => {
        const filteredArray = getState().toWatchMovies.filter(
        ...
        if (!filteredArray.length) {
          ...
          if (getState().toWatchMovies.length) {
            order =
              getState().toWatchMovies[getState().toWatchMovies.length - 1]
                .order + 1;
          }
          myJson.order = order;
...
export const getBigMovie = databaseID => {
  return (dispatch, getState) => {
    ...
    if (!filteredArray.length) {
      filteredArray = getState().watchedMovies.filter(
        movie => movie.id === databaseID
      );
    }
...
export const filterGenre = genre => {
  return (dispatch, getState) => {
    ...
    const filteredArray = getState().toWatchMovies.filter(item =>
      item.Genre.toLowerCase().includes(genre.toLowerCase())
    );
    dispatch(updateGenreFilteredData(filteredArray));
    myElement.firstChild.nodeValue = `Genre: ${genre}`;
    const resetFiltersBtn = document.querySelector(".reset-filters");
...
export const moveUp = order => {
  return (dispatch, getState) => {
    const firstIndex = getState().toWatchMovies.findIndex(
              ...
              .then(response => {
                if (response.ok) {
                  Communicators.Fetch()
                    .then(response => {
                      const formatedData =
                        formatData(response).sort(sortMovies);
                      const genres = takeGenres(formatedData);
                      dispatch(updateToWatchData(formatedData, genres));
                    })
                    .catch(error => console.error(`Error: ${error}`));
